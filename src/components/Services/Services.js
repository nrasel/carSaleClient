...
const Services = () => {
  ...
  //    https://hidden-temple-16176.herokuapp.com/homeCars

  useEffect(() => {
    fetch("https://odd-puce-cygnet-hat.cyclic.app/homeCars")
      .then((res) => res.json())
  ...
  return (
    <div className="bg-light py-5">
      <div className="container mt-4">
        <h2 style={{ color: "#052046", fontSize: "40px" }}>
          Buy Your Dream Car
        </h2>
        <p>
            ...
            {cars.map((car) => (
              <div key={car._id} className="col">
                <div
                  ...
                  className="card h-100 card-style border-0 text-start"
                >
                  <img
                    src={car.img}
                  ...
                  <div className="card-body">
                    ...
                    <span className="flat-icon">
                      <img className="flat-icon-width" src={calender} alt="" />{" "}
                      Petrol
                    </span>

                    <span className="flat-icon">
                      <img className="flat-icon-width" src={petrol} alt="" />{" "}
                      2021
                    </span>
                  </div>
                  <NavLink
                    style={{ backgroundColor: "#aaa9d6", color: "#2c4964" }}
                    className="btn btn-lg block"
                    to={`/orderPlace/${car._id}`}
                  >
